...
const App = () => {
 ...
 const API_KEY='d3b8c7bf1ad78f57c8d6a1098820313b'
  ...
  useEffect(() => {
    if(city.trim().length >= 3 && !weather){
      const timer = setTimeout(() => fetchSuggesions(city), 500); 
      return () => clearTimeout(timer);
    }
    setSuggestion([]);
  }, [city, weather]);

  //fetch 5 locations suggestion from api and updates
  const fetchSuggesions = async (query) => {
    try{
...
//weather exists function check and return an object
  ...
  <div className=' flex items-center justify-center p-6 min-h-screen'>
    ...
    {!weather ? (
      <form onSubmit={handleSearch} className=' flex flex-col relative'>
        <input value={city} onChange={(e) => setCity(e.target.value)} placeholder='Enter City or Country (min 3 letters)' className=' mb-4 p-3 rounded border border-white bg-transparent text-white placeholder-white focus:outline-none
         focus:border-blue-300 transition-300' />
         {suggestion.length > 0 && (
      ...
      <div className=' mt-6 text-center transition-opacity duration-500'>
        <button onClick={() => { setWeather(null); setCity('')}}
          className =' mb-4 bg-yellow-600 hover:bg-green-700 text-white font-semibold py-1 px-3 rounded transition-colors cursor-pointer'>
            New Search
        </button>
